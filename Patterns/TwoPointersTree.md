...
│       └── 🪣 Bucket + Iterator (Multi-Sequence Optimization)
...
### 5. 🪣 Bucket + Iterator (Multi-Sequence Optimization)

**🎯 Movement**: Single pointer through main sequence, multiple iterators grouped by current state

**🎨 Use Case**: Multiple sequences against one main sequence, avoiding redundant scanning

**⚡ Performance**: Transforms O(n × m) to O(n + total_chars) for multiple subsequence problems

```jsx
// 📋 Template for Bucket + Iterator
function processMultipleSequences(mainSeq, sequences) {
    // 🪣 Initialize buckets for each possible character
    const buckets = new Map();
    for (let i = 0; i < 26; i++) {
        buckets.set(String.fromCharCode('a'.charCodeAt(0) + i), []);
    }
    
    // 🎯 Create iterators and place in initial buckets
    ...
    // 📍 Place each iterator in bucket for its first character
    ...
    // 🚀 Single pass through main sequence
    for (const char of mainSeq) {
        ...
        for (const iterator of currentBucket) {
            ...
            if (iterator.position >= iterator.sequence.length) {
                iterator.isComplete = true; // ✅ Sequence completed
            } else {
...
**💡 Why it works**: Instead of checking each sequence individually (redundant scanning), group sequences by their current "waiting" character, then process all relevant sequences simultaneously in one pass.

**🎯 Key Insight**: 
- **Basic Two Pointers**: One sequence vs. one sequence → O(n + m)
- **Naive Multi-sequence**: Multiple sequences vs. one → O(n × sequences)  
- **Bucket + Iterator**: Multiple sequences vs. one → O(n + total_chars)
...
| "Count how many words..." | 🪣 Bucket + Iterator | Number of Matching Subsequences |
| "Multiple sequences vs one..." | 🪣 Bucket + Iterator | Batch Processing Problems |
...
| 🪣 **Bucket + Iterator** | `O(n+total)` | `O(sequences)` | Multi-sequence problems | ✅ None |
...
1. **🎲 3Sum** *(Opposite Direction + Loop)*
2. **🔤 Longest Substring Without Repeating Characters** *(Sliding Window)*
3. **🔄 Linked List Cycle** *(Fast & Slow)*
4. **✅ Is Subsequence** *(Subsequence Matching)*

### 🌳 **Advanced Level** (Complex applications)

1. **🐪 Camelcase Matching** *(Subsequence Matching with Rules)*
2. **🎯 4Sum** *(Multiple Pointers)*
3. **🪟 Minimum Window Substring** *(Complex Sliding Window)*
4. **🏺 Trapping Rain Water** *(Advanced Opposite Direction)*
